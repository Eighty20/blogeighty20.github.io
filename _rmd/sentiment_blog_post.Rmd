...
## this code block will not be visible in the final html ##
...
```{r combine_names_dates_poems, eval = F}
...
```{r, include = F}
poemDataFrame <- readRDS("Poetry/poemData.RDS")
```
...
```{r call_poet_functions, message = FALSE, eval = F}
...
```{r, include = F}
poets <- readRDS("Poetry/poets.RDS")
names(poets) <- c("yeats", "eliot", "frost", "wallace", "cummings", "lawrence")
list2env(poets, globalenv())
```
...
completedataFramebing <- poets %>%
  map(~.x %>% .[[2]] %>% select(name, date) %>% distinct) %>% 
  Map(cbind,., poet = as.list(names(poets))) %>% 
  reduce(rbind) %>% 
  filter(date >= 1890) 

completedataFramebing %>% 
  ggplot(., aes(date, fill = poet)) +
  geom_bar() +
  ...
  ylab("Frequency") +
  ggtitle("Bar Plots for Number of Poems Per Year") +
...
poets %>% 
  ...
  summarize(score = mean(scores)) %>% 
  arrange(score) %>%  
  mutate(poet = factor(poet, poet)) %>% 
  ggplot(., aes(x = poet, y = score, fill = poet)) +
  ...
  theme(axis.title.y = element_text(margin = margin(0, 10, 0, 0))) +
  ylab("Score") +
  ggtitle("Who is the saddest of them all?") +
  theme(plot.title = element_text(size = 14))
```
